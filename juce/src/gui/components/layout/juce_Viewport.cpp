...
//==============================================================================
void Viewport::addListener (ViewportListener* const newListener) throw()
{
    viewportListeners.addIfNotAlreadyThere (newListener);
}

void Viewport::removeListener (ViewportListener* const listenerToRemove) throw()
{
    viewportListeners.removeValue (listenerToRemove);
}

void Viewport::notifyComponentChanged ()
{
    for (int i = viewportListeners.size(); --i >= 0;)
    {
        ((ViewportListener*) viewportListeners.getUnchecked (i))
