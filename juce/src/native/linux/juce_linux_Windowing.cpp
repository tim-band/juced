...
// compiled on its own).
 ...
 #if JUCE_BIG_ENDIAN
  #define JUCE_GLX_PIXEL_FORMAT GL_RGBA
 #else
  #define JUCE_GLX_PIXEL_FORMAT GL_BGRA_EXT
 #endif
...
// our double buffered RGBA attributes
...
class GlxSharedConfiguration : public DeletedAtShutdown
{
public:

    GlxSharedConfiguration ()
        : isAvailable (false),
          pVisInfo (0),
          pFBConfigs (0),
          iNumOfFBConfigs (0),
          iOffsetOfFBConfigs (-1)
    {
        // check configuration of XRender
        trapErrors();

        // test xrender version
        int iRenderEventBase, iRenderErrorBase;
        if (! XRenderQueryExtension (display, &iRenderEventBase, &iRenderErrorBase))
        {
            untrapErrors();
            return;
        }
        ...
        if (! glXQueryVersion (display, &iMajorVersion, &iMinorVersion)
            || (iMajorVersion < 1)
            || (iMajorVersion == 1 && iMinorVersion <= 2))
        {
            untrapErrors();
            return;
        }

        // look for a framebuffer configuration, get a double buffered config
        pFBConfigs = glXChooseFBConfig (display,
        ...
        if (pFBConfigs)
        {
            ...
            {
                XVisualInfo* visualInfo = glXGetVisualFromFBConfig (display, pFBConfigs[i]);
                if (visualInfo)
                {
                    XRenderPictFormat* pictureFormat = XRenderFindVisualFormat (display, visualInfo->visual);
                    if (pictureFormat)
                    {
                        if (pictureFormat->direct.alphaMask > 0)
                        {
                            DBG ("Strike, found a GLX visual with alpha support !");
                            pVisInfo = visualInfo;
                            iOffsetOfFBConfigs = i;
                            isAvailable = true;
                            break;
                        }
                    }
    ...
    {
        jassert (pFBConfigs != 0)

        return pFBConfigs [iOffsetOfFBConfigs];
    }

    juce_DeclareSingleton (GlxSharedConfiguration, false)
...
protected:
    ...
    int iNumOfFBConfigs;
    int iOffsetOfFBConfigs;
};

juce_ImplementSingleton (GlxSharedConfiguration);
...
private:
    ...
    public:
        ...
        void performAnyPendingRepaintsNow()
        {
            ...
            if (peer->isGlxWindow ())
            {
                ...
                peer->getBounds (x, y, width, height);

                if (glxFrameBufferImage == 0
                    || glxFrameBufferImage->getWidth() < width
                    || glxFrameBufferImage->getHeight() < height)
                {
                    delete glxFrameBufferImage;

                    glxFrameBufferImage = new Image (Image::ARGB, width, height, true);
                }

                LowLevelGraphicsSoftwareRenderer context (*glxFrameBufferImage);

                peer->handlePaint (context);

                int stride, pixStride;
                const void* pixels = glxFrameBufferImage->lockPixelDataReadOnly (0, 0,
                                                                                 width, height,
                                                                                 stride, pixStride);

                peer->redrawGlx (width, height, pixels);

                glxFrameBufferImage->releasePixelDataReadOnly (pixels);
            }
            else
#endif
            {
    ...
    {
        jassert (glxContext != 0);

        return glXMakeCurrent (display, windowH, glxContext)
                && XSync (display, False);
    }
    ...
    bool isGlxWindow ()
    {
        return glxWindow != 0 && glxContext != 0;
    }
    ...
    void resizeGlx (const int w, const int h)
    {
        glViewport (0, 0, w, h);
        ...
        glOrtho (0.0, (GLfloat) w, (GLfloat) h, 0.0, -1.0, 1.0);
        ...
        if (1)
            glOrtho ((GLdouble) -w / 2, (GLdouble) w / 2,
                     (GLdouble) -h / 2, (GLdouble) h / 2,
                     Z_NEAR, Z_FAR);
        else
            gluPerspective (2.0f * FOVY,
    ...
    void redrawGlx (const int w, const int h, const void* pixels)
    {
        makeActive();
        ...
        glTexImage2D (GL_TEXTURE_2D, 0, 4, w, h,
                      0, JUCE_GLX_PIXEL_FORMAT, GL_UNSIGNED_BYTE, pixels);
        ...
        glXSwapBuffers (display, windowH);
    ...
    void skipTaskbarButton (Window wndH)
    {
        // check gtk/gdk/x11/gdkwindow-x11.c : gdk_window_set_skip_taskbar_hint
        Atom hints = XInternAtom (display, "_NET_WM_STATE", False);
        Atom windowSkipTaskbar = XInternAtom (display, "_NET_WM_STATE_SKIP_TASKBAR", False);

        XChangeProperty (display, wndH, hints, XA_ATOM, 32,
                         PropModeAppend, (unsigned char*) &windowSkipTaskbar, 1);
    }
    ...
    void createWindow()
    {
        ...
        // what if we need a semi transparent window
        Window wndH;
        bool fallbackWindowMode = true;

        if ((styleFlags & windowIsSemiTransparent) != 0
            && Desktop::canUseSemiTransparentWindows())
        {
#if JUCE_OPENGL && JUCE_USE_GLX
            // Attempt to create a 32-bit window on the default screen.  If this is not
            // possible then use the fallbackWindowMode

            GlxSharedConfiguration* glxConfig = GlxSharedConfiguration::getInstance ();

            XVisualInfo* visualInfo = glxConfig->getXVisualInfo();
            GLXFBConfig renderFBConfig = glxConfig->getGLXFBConfig();
            ...
            wndH = XCreateWindow (display, root,
                                  0, 0, 1, 1,
            ...
            glxWindow = glXCreateWindow (display, renderFBConfig, wndH, 0);

            // make the current context drawing into our window
            glXMakeContextCurrent (display, glxWindow, glxWindow, glxContext);

            // the window must be mapped before initializing the gl
            
            // TODO - this makes things flashon the taskbar.
//            XMapWindow (display, wndH);
//            XSync (display, False);

            // this has to be done once
//            initializeGlx ();

            fallbackWindowMode = false;
#endif
        }
        ...
        // Skip taskbar button for the window
        if ((styleFlags & windowAppearsOnTaskbar) == 0)
            skipTaskbarButton (wndH);
    ...
    {
#if JUCE_OPENGL && JUCE_USE_GLX
        if (glxContext || glxWindow)
        {
            //glXMakeContextCurrent (display, glxWindow, None, NULL);
...
{
#if JUCE_OPENGL && JUCE_USE_GLX
    return GlxSharedConfiguration::getInstance ()->isGlxAvailable ();
#else
    return false;
#endif
}
